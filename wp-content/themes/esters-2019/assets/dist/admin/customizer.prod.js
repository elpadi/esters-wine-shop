...
if(typeof noGlobal==="undefined"){window.jQuery=window.$=jQuery}return jQuery})},{}],2:[function(require,module,exports){class Customizer{constructor(){if("customize"in wp)setTimeout(()=>this.init(),3000)}moveSlide(newIndex,oldIndex){for(let k of["title","image","byline","page"]){let newname=`home_slides_${newIndex}_${k}`;if(oldIndex>0){let oldname=`home_slides_${oldIndex}_${k}`;wp.customize.value(newname)(wp.customize.value(oldname)())}else{wp.customize.value(newname)(false)}}}rolloverSlides(){console.log("Customizer.rolloverSlides");for(let i=5;i>=0;i--)this.moveSlide(i,i-1)}initSlideRolloverControl(){let rollCtrl=wp.customize.control("home_slides_rollover");if(rollCtrl){console.log("HomePage.initSlideRolloverControl",rollCtrl);rollCtrl.container.find("input").attr("value","Insert First Slide").addClass("button").on("click",e=>this.rolloverSlides())}}init(){console.log("Customizer.init");this.initSlideRolloverControl()}}module.exports=Customizer},{}],3:[function(require,module,exports){(function(global){(function(){const $=require("jquery");const Obj=require("./obj");const Str=require("./str");const DOM=require("./dom");const Media=require("./media");const WP=require("./wp");const Theme=require("./theme");const GlobalModule=require("../modules/global");class App{constructor(){this.modules={global:GlobalModule};this.instances={};this.ENV=global.JS_ENV;this.hasCreatedInstances=false}init(){window.addEventListener("DOMContentLoaded",e=>this.onDocReady());window.addEventListener("load",e=>this.load());window.addEventListener("resize",e=>this.resize())}load(){setTimeout(this.resize.bind(this),100);setTimeout(()=>document.body.classList.add("content-loaded"),200);this.vw=window.innerWidth;this.vh=window.innerHeight;this.dispatch("load")}isElementInViewport(rect,threshold=0.3){return DOM.isElementInViewport(rect,this.vw,this.vh,threshold)}getDims(obj){return Media.getDims(obj)}fixTopPositions(){return DOM.fixTopPositions()}slugify(s,glue="_"){return Str.slugify(s,glue)}queryStringToObject(s){return Str.queryStringToObject(s)}removeFileExtension(path){return Str.removeFileExtension(path)}objSet(name,val,obj){return Obj.set(name,val,obj)}ajax(data,method="GET"){return WP.ajax(data,method)}apiRequest(endpoint,data){return WP.apiRequest(endpoint,data)}fetchPosts(type="pages",params={}){return WP.fetchPosts(type,params)}fetchIcon(name,dir="icons"){return Theme.fetchAsset(`${dir}/${name}.svg`)}resize(){this.vw=window.innerWidth;this.vh=window.innerHeight;this.dispatch("resize",this.vw,this.vh)}addModule(name,module){console.log("App.addModule",name,this.hasCreatedInstances);if(this.hasCreatedInstances){if(name in this.instances)throw`Module name "${name}" is already in use.`;this.createModuleInstance(module,name)}else{if(name in this.modules)throw`Module name "${name}" is already in use.`;this.modules[name]=module}return this}dispatch(evtName,...vars){console.log("App.dispatch",evtName,vars);for(let name of Object.keys(this.instances)){let m=this.instances[name];if(evtName in m&&typeof m[evtName]=="function")m[evtName].apply(m,vars)}}instanceInvoke(name,fn,...args){if(name in this.instances&&fn in this.instances[name]){this.instances[name][fn].apply(this.instances[name],args)}}createModuleInstance(module,name){console.log("App.createModuleInstance",name,module);this.instances[name]=new module(this);this.instances[name].app=this}onDocReady(){this.hasCreatedInstances=true;console.log("App.onDocReady",this.modules,document.body.className);for(let name of Object.keys(this.modules))this.createModuleInstance(this.modules[name],name)}}module.exports=App}).call(this)}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{})},{"../modules/global":12,"./dom":4,"./media":5,"./obj":6,"./str":7,"./theme":8,"./wp":9,"jquery":1}],4:[function(require,module,exports){class DOM{static forceTargetBlank(){for(let l of $$(".external-link").concat([/* other external links */]))l.target="_blank"}static fixTopPositions(){let t=document.querySelector("#masthead").offsetHeight;for(let s of["#page","#tome-menu"])document.querySelector(s).style.paddingTop=t+"px";for(let s of["#menu-burger"])document.querySelector(s).style.marginTop=t+"px"}static isElementInViewport(rect,vw,vh,threshold=0.3){return rect.top<vh*(1-threshold)&&rect.bottom>vh*threshold}}module.exports=DOM},{}],5:[function(require,module,exports){class Media{static getDims(obj){let dims,nat=()=>dims={w:obj.naturalWidth,h:obj.naturalHeight},r=()=>dims.r=dims.w/dims.h;if(obj.naturalWidth)nat();else{let matches=obj.src?obj.src.match(/([0-9]+)x([0-9]+)/):null;if(matches){dims={w:Number(matches[1]),h:Number(matches[2])}}}return new Promise((resolve,reject)=>{if(dims){r();resolve(dims)}else{obj.addEventListener("load",e=>{nat();r();resolve(dims)})}})}}module.exports=Media},{}],6:[function(require,module,exports){class Obj{static objSet(name,val,obj){if(obj==undefined)obj={};obj[name]=val;return obj}}module.exports=Obj},{}],7:[function(require,module,exports){class Str{static slugify(s,glue="_"){return s.trim().toLowerCase().replace(/[^0-9a-z]+/g,glue).replace(new RegExp(`(${glue})+`,"g"),glue)}static queryStringToObject(s){let d={},params=new URLSearchParams(s);for(let n of params.entries())d[n[0]]=n[1];return d}static removeFileExtension(path){return path.replace(/\.[a-zA-Z0-9]+$/,"")}}module.exports=Str},{}],8:[function(require,module,exports){const $=require("jquery");const WP=require("./wp");class Theme{static fetchAsset(path){return WP.ajax({action:"get_asset_contents",path:path})}}module.exports=Theme},{"./wp":9,"jquery":1}],9:[function(require,module,exports){const $=require("jquery");class WP{static apiRequest(endpoint,data){if(!("api"in wp)||!("apiRequest"in wp))return Promise.reject("WP API library is not loaded.");return wp.apiRequest({namespace:wp.api.versionString,endpoint:endpoint,data:data})}static fetchPosts(type="pages",params={}){return WP.apiRequest(type,params)}static ajax(data,method="GET"){if(!data||!("action"in data)||typeof data.action!="string"){throw"Invalid argument for ajax data. Invalid or missing action name."}// prepend theme name to ajax action
let prefix=app.ENV.THEME.NAME+"_";if(data.action.indexOf(prefix)!=0)data.action=prefix+data.action;console.log("AdminApp.ajax",data,method);return $[method.toLowerCase()](app.ENV.URLS.AJAX,data,resp=>{if(typeof resp=="object"){if("success"in resp&&"data"in resp){if(typeof resp.data=="object"){console.log("App.ajax",resp,data,method)}}}})}}module.exports=WP},{"jquery":1}],10:[function(require,module,exports){const App=require("../../base/app");const app=new App;window.app=app;app.modules["customizer"]=require("../../admin/customizer/customizer");app.init()},{"../../admin/customizer/customizer":2,"../../base/app":3}],11:[function(require,module,exports){const $=require("jquery");class Esters{constructor(){this.burgerMQL=window.matchMedia("(min-width: 768px)");this.initSiteElementClasses();this.initSearchForm();this.initBurger()}initSiteElementClasses(){$("#theme-header__online-shops").addClass("double-border");$(".button").addClass("btn").removeClass("button");$(".woocommerce-Addresses h3").addClass("star-heading");if(app.ENV.USER.ID&&/\/my-account\/?/.test(location.pathname))document.body.classList.add("my-account-dashboard")}initSearchForm(){$(".search-form").on("submit",e=>{if(e.target.s.value==""){e.preventDefault();e.target.classList.add("active")}});if(!this.burgerMQL.matches){$(".search-form").addClass("active")}}resize(){if(this.burgerMQL.matches){this.unsetBurgerTransitionStyles()}else{if(this.isCollapsed())this.setBurgerTransitionStyles()}}load(){if(!this.burgerMQL.matches){setTimeout(this.collapse,100)}}collapse(){document.body.dataset.isBurgerExpanded="false"}expand(){document.body.dataset.isBurgerExpanded="true"}isCollapsed(){return document.body.dataset.isBurgerExpanded!=="true"}isExpanded(){return document.body.dataset.isBurgerExpanded==="true"}unsetBurgerTransitionStyles(){["#theme-nav","#masthead"].forEach(s=>document.querySelector(s).setAttribute("style",""))}setBurgerTransitionStyles(){let nav=document.querySelector("#theme-nav"),header=document.querySelector("#masthead"),headerRect=header.getBoundingClientRect();nav.style.top=`${headerRect.bottom+window.scrollY}px`;nav.style.transform=`translateY(calc(-100% - ${headerRect.top+window.scrollY}px - ${headerRect.height}px))`;header.style.marginBottom=`${nav.offsetHeight}px`}initBurger(){$(document).on("click",".burger-expand-btn",e=>this.expand());$(document).on("click",".burger-collapse-btn",e=>this.collapse());this.burgerMQL.addListener(e=>{if(e.matches)this.expand();else this.collapse()})}}module.exports=Esters},{"jquery":1}],12:[function(require,module,exports){const $=require("jquery");const Esters=require("./esters");const Modal=require("./modal");class GlobalModule{constructor(){app.addModule("esters",Esters);app.addModule("modal",Modal)}}module.exports=GlobalModule},{"./esters":11,"./modal":13,"jquery":1}],13:[function(require,module,exports){const $=require("jquery");class Modal{constructor(){this.hasModalCheck();$(document).on("click",".modal-btn",this.onModalBtnClick.bind(this)).on("click",".modal__close-btn",this.onCloseBtnClick.bind(this))}hasModalCheck(){setTimeout(()=>{document.body.dataset.hasModal=document.querySelectorAll(".modal.visible").length?"true":"false"},50)}onCloseBtnClick(e){$(e.target).closest(".modal").removeClass("visible");this.hasModalCheck()}onModalBtnClick(e){$(`#${e.currentTarget.dataset.modalId}-modal`).addClass("visible");this.hasModalCheck()}}module.exports=Modal},{"jquery":1}]},{},[10]);
