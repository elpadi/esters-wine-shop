...
if(typeof noGlobal==="undefined"){window.jQuery=window.$=jQuery}return jQuery})},{}],2:[function(require,module,exports){const ShopLandingPage=require("../../../modules/pages/shop-landing");if("app"in window){window.app.addModule("landingpage",ShopLandingPage)}else console.error("App is not loaded")},{"../../../modules/pages/shop-landing":3}],3:[function(require,module,exports){const $=require("jquery");class ShopLandingPage{constructor(){document.querySelector("#content").style.backgroundImage=`url(${app.ENV.URLS.THEME}/assets/img/bg-patterns/shop.jpg)`;this.cols=$(".wp-block-column").addClass("double-border");this.cols=$(".wc-block-grid__product-link").addClass("btn");/*
		this.addClubButtons();
		this.addShopButtons();
		*/}addButtons(colIndex,btns){let btnsHTML=btns.map(btn=>`<a class="btn" href="${btn.href}">${btn.content}</a>`).join("");this.cols.get(colIndex).insertAdjacentHTML("beforeend",`<p class="shop-btns">${btnsHTML}</p>`)}addClubButtons(){}addShopButtons(){this.addButtons(1,[{href:"/shop",content:"Shop Now"}])}initSlider(){let element=$("#home-slideshow");if(element.children().length>1)new Slider(element)}}module.exports=ShopLandingPage},{"jquery":1}]},{},[2]);
