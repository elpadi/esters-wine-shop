...
if(typeof noGlobal==="undefined"){window.jQuery=window.$=jQuery}return jQuery})},{}],2:[function(require,module,exports){const CalendarPage=require("../../../modules/pages/calendar");if("app"in window){window.app.addModule("calendarpage",CalendarPage)}else console.error("App is not loaded")},{"../../../modules/pages/calendar":5}],3:[function(require,module,exports){const Listing=require("./listing");class EventsListing extends Listing{constructor(){super();this.ajaxAction="get_calendar_events"}getPostHTML(p){let dateParts=new Date(p.real_date).toUTCString().split(" ");// e.g. "Tue, 01 Oct 2019 00:00:00 GMT"
return`<article id="post-${p.id}" class="entry type-${p.type}">
		...
		</article>`}}module.exports=EventsListing},{"./listing":4}],4:[function(require,module,exports){class Listing{constructor(){}fetch(){return("apiEndpoint"in this?app.fetchPosts(this.apiEndpoint,this.apiParams):app.ajax({action:this.ajaxAction})).then(response=>{let posts=Array.isArray(response)?response:response?"success"in response&&response.success?response.data:[]:[];console.log("Listing.fetch",posts);this.posts=posts})}getPostsHTML(){return this.posts.map(p=>this.getPostHTML(p)).join("")}getPostHTML(p){return`<article id="post-${p.id}" class="entry type-${p.type}">
			<header class="entry-header">
				<h2 class="entry-title">${p.title.rendered}</h2>
			</header>
			<div class="entry-content">${p.content.rendered}</div>
		</article>`}}module.exports=Listing},{}],5:[function(require,module,exports){const $=require("jquery");const EventsListing=require("../components/posts/events-listing");class CalendarPage{constructor(){document.querySelector(".page--calendar #content").style.backgroundImage=`url(${app.ENV.URLS.THEME}/assets/img/bg-patterns/menu.jpg)`}load(){this.addEvents()}addEvents(){this.events=new EventsListing;this.events.fetch().then(()=>{this.addEventsTitle();this.addEventsContent()})}addEventsTitle(){document.querySelector(".page--calendar .entry-content").insertAdjacentHTML("beforeend",`<h2>What's Upcoming At Esters</h2>`)}addEventsContent(){document.querySelector(".page--calendar article.entry").insertAdjacentHTML("afterend",this.events.getPostsHTML());setTimeout(()=>{$(".event__desc p").filter((i,p)=>p.innerText.trim()=="").remove()},100)}}module.exports=CalendarPage},{"../components/posts/events-listing":3,"jquery":1}]},{},[2]);
