...
class Theme extends Frontend
{
    ...
    public function updateBodyClasses($classes)
    {
        ...
        if (is_page()) {
            if (
                !is_front_page() && !in_array($post->post_name, [
                'our-menu',
                'private-events',
                ])
            ) {
            ...
            if (in_array($post->post_name, ['our-menu','shop-landing','calendar'])) {
                $classes[] = 'content-bg-pattern';
            }

            if (in_array($post->post_name, ['terms-and-conditions','privacy-policy','terms-of-use'])) {
                $classes[] = 'legal-page';
            }
        }
    ...
    protected function enqueueFrontScripts($vars)
    {
        ...
        if (empty($js_names)) {
            $js_names[] = $assetFilename('js', 'page');
        }
