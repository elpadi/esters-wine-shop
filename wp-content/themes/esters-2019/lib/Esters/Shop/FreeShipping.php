...
class FreeShipping
{
    ...
    {
        return F\every($package['contents'], function ($content) {
            return $this->productHasFreeShipping($content['product_id']);
        });
    }
    ...
    {
        if ($rate->get_method_id() == 'fedex' && $this->isFedexRateFree($rate)) {
            return '0.00';
        }
        return $cost;
    }
    ...
    protected function isFedexRateFree($rate)
    {
        ...
        foreach ($meta as $key => $val) {
            if (!preg_match('/^Package [0-9]+$/', $key)) {
                continue;
            } else {
                $hasFreePackages = true;
            }

            // if any package is not free, fail
            if (strpos($val, '1 × 1 × 1 (in) 1lbs × ') !== 0) {
                return false;
            }
        }
